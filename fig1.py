...
for system in systems.systems.keys():
 ...
 Lifetime_Iter = (systems.get_dos(system,J=J,MQN=MQN,nmax=nmax,vmax=vmax,energy_offset=collision_energy)[1] for collision_energy in collision_energies)
 Lifetime_ZeroEnergy_Iter = (systems.get_dos(system,J=J,MQN=MQN,nmax=nmax,vmax=vmax)[1] for collision_energy in collision_energies)
 NumOpen  = np.fromiter(NumOpen_Iter,np.int_)
 Lifetime = np.fromiter(Lifetime_Iter,np.float)
 Lifetime_ZeroEnergy = np.fromiter(Lifetime_ZeroEnergy_Iter,np.float)
 plt.plot(collision_energies,Lifetime/NumOpen,label='Helium+'+system.split('_')[1].capitalize())
 plt.plot(collision_energies,Lifetime_ZeroEnergy/NumOpen,'--')
