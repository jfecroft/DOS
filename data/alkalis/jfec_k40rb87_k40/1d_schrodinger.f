...
C     DATA ZMA/1836.9822D0/
C     DATA ZMB/34629.61319D0/
C     DATA R0/1.7329D0/
...
C     NAMELIST DATA
      NAMELIST /TTDATA/ ZMA, ZMB, R0, BETA, DHART, NPRIN, NWRIT, L,
     &RMAX, RMIN, MORLJ, C6, C8, C10, C12
      READ(5,NML=TTDATA)
...
C.....Set up grid
      ...
      IF( (C12/C6)**(1.0/6.0) < RMIN) THEN
       WRITE(6,*) 'If using LJ poential rmin too small'
      ENDIF
C      RMIN=R0*0.2D0
C      RMAX=R0*3.0D0
      ZL=(RMAX-RMIN)
...
C.....Find the potential value at x
      CALL VSUB_MLR3(X,VV,L,ZMU)
      POT(I) = VV
      IF(VMIN .gt. VV) VMIN = VV
...
12    FORMAT(/)
      ...
      DO 009 I=1,NWRIT  
       ...
       IF (WCH(I) .LT. 0.0D0 .AND. WCH(I) .GT. VMIN) THEN
        WRITE(10,*) I-1.0, WCH(I)
       ENDIF
 009  CONTINUE
      WRITE(6,12)
